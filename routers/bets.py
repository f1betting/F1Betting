...
def get_bet(username: str, race: int):
    user = database.database["Users"].find_one({"username": username.lower()})

    if not user:
        return JSONResponse(status_code=404, content=create_message("User not found"))

    bet = database.database["Bets"].find_one({"username": user["username"], "round": race})
...
def create_bet(bet: BaseBet):
    ...
    user = database.database["Users"].find_one({"username": bet["username"]})

    if not user:
        return JSONResponse(status_code=404, content=create_message("User not found"))

    if list(database.database["Bets"].find(
    ...
    new_bet = database.database["Bets"].insert_one(bet)

    created_bet = database.database["Bets"].find_one({"_id": new_bet.inserted_id})
...
def edit_bet(username: str, race: int, season: int, p1: str, p2: str, p3: str):
    ...
    bet = database.database["Bets"].find_one({"username": user["username"], "round": race, "season": season})
    ...
    database.database["Bets"].update_one({"_id": bet["_id"]}, {"$set": {
        "p1": p1.upper(),
        "p2": p2.upper(),
        "p3": p3.upper()
    }})
...
def delete_bet(username: str, race: int):
    ...
    database.database["Bets"].delete_one({"_id": bet["_id"]})
